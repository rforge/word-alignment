preparData <-
function(file1, file2, nrec = -1, mlen = 40, lus = FALSE, lut = TRUE, all = FALSE, word_align = FALSE, la = TRUE, map = c ('ef', 'fe'))
{
map = match.arg(map)

s_sen = t_sen = aa = t = c()

s_sen = readLines (con <- file(file1), encoding = 'UTF-8', n = nrec)
close(con)

t_sen = readLines (con <- file(file2), encoding = 'UTF-8', n = nrec)
close(con)
...
if (map == 'ef')  aa [,1] = paste ('null', aa [,1])
if (map == 'fe')  aa [,2] = paste ('null', aa [,2])
