consExcel <-
function(tst.set_sorc, tst.set_trgt, gold = TRUE, aligns = FALSE, out1 = "gold.xlsx", out2 = "align.xlsx",nrec = -1, maxlen = 40, ul_s = FALSE, ul_t = TRUE)
{
p1 = preparData (tst.set_sorc, tst.set_trgt, nrec = nrec, maxlen = maxlen, ul_s = ul_s, ul_t = ul_t)
len = p1 $ length1
p1 = unlist (p1, recursive = FALSE)
p1 = sapply(2 : length(p1), function(x) c('null', p1[[x]]))
